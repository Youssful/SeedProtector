...
- `/seedprotect audit` command to check if any structures are still using default seeds
...
3. Run `/seedprotect randomize` (from console or in-game with OP).
...
7. Run `/seedprotect audit` to confirm everything is randomized.

---

### Commands & Permissions

| Command | Subcommands / Aliases | Permission | Default | Description |
|---|---|---|---|---|
| `/seedprotect audit` | `/sp audit`, `/seedguard check`, `/sp status` | `seedprotect.audit` | OP | Checks all structure seeds and reports any defaults |
| `/seedprotect randomize` | `/sp randomize`, `/seedguard shuffle`, `/sp scramble` | `seedprotect.randomize` | OP | Writes randomized seeds to `spigot.yml` and Paper config |
| `/seedprotect help` | `/sp help` | None | Everyone | Shows command usage |

Command aliases: `/seedguard`, `/seedprotector`, `/seedshield`, `/sp`.
